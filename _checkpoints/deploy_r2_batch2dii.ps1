# deploy_r2_batch2dii.ps1  -- R-2 batch 2d-ii: HR/Payroll + worker-shared cluster
...
$ok = ($txt -match "hr\.payroll\.run") -and ($txt -match "hr\.attendance\.view") -and ($txt -match "settings\.devices\.manage")
Write-Host ("batch-2d-ii markers present: " + $ok)
...
Write-Host "Batch 2d-ii deployed (HR/Payroll + worker-shared cluster)."
