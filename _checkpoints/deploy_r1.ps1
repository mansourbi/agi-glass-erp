# ============================================================================
#  BLOCK R-1 - Roles & Privileges FOUNDATION (no enforcement yet; nothing breaks)
#  Adds middleware/permissions.js + routes/access.js, registers /api/access in
#  server.js (UTF-8-safe, backed up), seeds Superadmin/Admin/Worker + maps users.
#  RUN AS ADMINISTRATOR.
# ============================================================================
$ts   = Get-Date -Format 'yyyyMMdd-HHmmss'
$srv  = 'C:\agi-server'
$mw   = Join-Path $srv 'middleware'
$rt   = Join-Path $srv 'routes'
$bk   = Join-Path $srv '_route_backups'
New-Item -ItemType Directory -Force -Path $bk | Out-Null
$serverPath = Join-Path $srv 'server.js'
$permPath   = Join-Path $mw 'permissions.js'
$accPath    = Join-Path $rt 'access.js'

# --- backups ---
Copy-Item $serverPath (Join-Path $bk "server.js.$ts.bak") -Force
if (Test-Path $permPath) { Copy-Item $permPath (Join-Path $bk "permissions.js.$ts.bak") -Force }
if (Test-Path $accPath)  { Copy-Item $accPath  (Join-Path $bk "access.js.$ts.bak")  -Force }

# --- write new module + route ---
$perm = @'
...
if(s.indexOf("/api/access")>=0){ console.log('SKIP: /api/access already registered'); process.exit(0); }
const anchor="app.use('/api/pricing_admin', require('./routes/pricing_admin'));";
const idx=s.indexOf(anchor);
if(idx<0){ console.error('ABORT: anchor (pricing_admin route) not found'); process.exit(2); }
let eol=s.indexOf('\n', idx+anchor.length);
if(eol<0) eol=idx+anchor.length;
const line="\napp.use('/api/access', require('./routes/access'));";
s=s.slice(0,eol)+line+s.slice(eol);
fs.writeFileSync(p,s,'utf8');
console.log('OK: registered /api/access after pricing_admin');

'@
Set-Content -Path $permPath -Value $perm  -Encoding ascii
Set-Content -Path $accPath  -Value $acc   -Encoding ascii
$patchPath = Join-Path $env:TEMP 'patch_server_r1.js'
Set-Content -Path $patchPath -Value $patch -Encoding ascii

# --- patch server.js (idempotent, UTF-8 preserving) ---
Push-Location $srv
& node $patchPath $serverPath
$patchRc = $LASTEXITCODE
Pop-Location
if ($patchRc -ne 0) { Write-Host 'ABORT: server.js patch failed; restoring.'; Copy-Item (Join-Path $bk "server.js.$ts.bak") $serverPath -Force; exit 1 }

# --- syntax checks (restore-on-fail) ---
Push-Location $srv
& node --check $permPath;   $c1=$LASTEXITCODE
& node --check $accPath;    $c2=$LASTEXITCODE
& node --check $serverPath; $c3=$LASTEXITCODE
Pop-Location
if ($c1 -ne 0 -or $c2 -ne 0 -or $c3 -ne 0) {
  Write-Host 'ABORT: syntax check failed; restoring server.js.'
  Copy-Item (Join-Path $bk "server.js.$ts.bak") $serverPath -Force
  exit 1
}
Write-Host 'Files written + server.js patched + syntax OK.'

# --- robust restart (access.js seeds roles on load) ---
...
# --- verify the seed ---
Push-Location $srv
& node -e "const db=require('./db'); const roles=db.prepare('SELECT id,name,superadmin,portal_access,workerapp_access FROM roles').all(); console.log('ROLES: '+roles.map(r=>r.id+':'+r.name+(r.superadmin?'(SA)':'')+' portal='+r.portal_access+' app='+r.workerapp_access).join(' | ')); const split=db.prepare('SELECT r.name nm, COUNT(*) n FROM workers w LEFT JOIN roles r ON r.id=w.role_id GROUP BY w.role_id').all(); console.log('USER MAP: '+split.map(x=>(x.nm||'UNASSIGNED')+':'+x.n).join('  ')); const ak=db.prepare(\"SELECT COUNT(*) c FROM role_permissions rp JOIN roles r ON r.id=rp.role_id WHERE r.name='Admin'\").get().c; console.log('Admin role keys: '+ak+' (expect 122)');"
Pop-Location
Write-Host 'R-1 foundation live. No existing route behavior changed.'
