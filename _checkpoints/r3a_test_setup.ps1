...
#  * role ZZ_R3A_TEST (portal) perms: portal.access, orders.access, cutting.access, tracking.access
#  * user zz_r3a/zzr3a123 (role string 'admin' bypasses device; role_id drives enforce + /api/access/me)
...
db.prepare("INSERT OR IGNORE INTO roles(name,description,is_system,superadmin,portal_access,workerapp_access) VALUES(?,?,0,0,1,0)")
  .run("ZZ_R3A_TEST","R-3a nav-gating verification role");
const rid = roleId("ZZ_R3A_TEST");
...
["portal.access","orders.access","cutting.access","tracking.access"].forEach(k => ins.run(rid, k));
const ph = bcrypt.hashSync("zzr3a123", 10);
db.prepare("DELETE FROM workers WHERE email=?").run("zz_r3a");
db.prepare("INSERT INTO workers(name,email,pass_hash,role,processes,is_active,role_id) VALUES(?,?,?,?,?,1,?)")
  .run("ZZ R3A Test","zz_r3a",ph,"admin","[]",rid);
console.log("READY: zz_r3a/zzr3a123 (role ZZ_R3A_TEST id "+rid+", perms portal+orders+cutting+tracking .access)");
'@
$tmp = "C:\agi-server\_zz_r3a_setup.js"
