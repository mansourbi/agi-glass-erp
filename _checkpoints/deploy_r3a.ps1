# deploy_r3a.ps1  -- R-3a: portal nav-section gating (frontend). Static-file edit, no service restart.
...
(function(){
  ...
  function firstAllowed(){
    for (var i=0;i<NAV_ORDER.length;i++){ var k=NAV_ORDER[i]; if (CAN(SECTION_PERM[k])) return k; }
    return null;
  }
  ...
  function applyGating(){
    ...
    var activeBtn = document.querySelector('[id^="nt-"].on');
    var activeKey = activeBtn ? activeBtn.id.slice(3) : null;
    if (!activeKey || !CAN(SECTION_PERM[activeKey])) {
      var f = firstAllowed();
      if (f && window.SP) window.SP(f);
    }
  }
  ...
  function wrapSP(){
    if (window.SP && !window.SP.__r3) {
      ...
      window.SP = function(k){
        ...
        if (p && !CAN(p)) { var f = firstAllowed(); return f ? _SP(f) : undefined; }
        return _SP.apply(this, arguments);
      };
      window.SP.__r3 = true;
      window.SP.__orig = _SP;
    }
  }

  // re-wrap (the app reassigns SP after load) + re-hide on each pass; cheap + idempotent
  function reconcile(){ if (!permsLoaded) return; wrapSP(); applyGating(); }
  ...
  function ready(){
    return document.getElementById('nt-dash') && typeof window.SP === 'function' && getToken();
  }
  var tries = 0;
  (function boot(){
    if (ready()) { start(); return; }
    if (tries++ < 150) setTimeout(boot, 200);
  })();
  // re-reconcile after the app finishes its own init (which can reassign SP)
  window.addEventListener('load', function(){
    setTimeout(function(){ if (permsLoaded) reconcile(); else start(); }, 300);
  });
})();
/* /R3-NAV-GATING v1 */
'@
...
Write-Host "R3a deployed (portal nav gating). Reload the portal (Ctrl+F5) to see it -- static file, no service restart needed."
