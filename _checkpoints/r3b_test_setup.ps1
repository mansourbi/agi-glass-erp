...
#  * role ZZ_R3B_TEST (portal) perms: portal.access, orders.access, cutting.access, tracking.access
#  * user zz_r3b/zzr3b123 (role string 'admin' bypasses device; role_id drives enforce + /api/access/me)
...
db.prepare("INSERT OR IGNORE INTO roles(name,description,is_system,superadmin,portal_access,workerapp_access) VALUES(?,?,0,0,1,0)")
  .run("ZZ_R3B_TEST","R-3b settings-gating verification role");
const rid = roleId("ZZ_R3B_TEST");
...
["portal.access","settings.access","settings.glassfamilies.manage","settings.factories.manage"].forEach(k => ins.run(rid, k));
const ph = bcrypt.hashSync("zzr3b123", 10);
db.prepare("DELETE FROM workers WHERE email=?").run("zz_r3b");
db.prepare("INSERT INTO workers(name,email,pass_hash,role,processes,is_active,role_id) VALUES(?,?,?,?,?,1,?)")
  .run("ZZ R3B Test","zz_r3b",ph,"admin","[]",rid);
console.log("READY: zz_r3b/zzr3b123 (role ZZ_R3B_TEST id "+rid+", perms settings.access+glassfamilies.manage+factories.manage)");
'@
$tmp = "C:\agi-server\_zz_r3b_setup.js"
