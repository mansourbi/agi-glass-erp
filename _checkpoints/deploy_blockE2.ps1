# ============================================================================
#  BLOCK E-2  -  Pricing engine, PREVIEW MODE (read-only endpoint).
#  Deploys routes/pricing2.js + registers it in server.js + restarts service.
#  Defensive: backs up server.js, node-checks BOTH files before restarting,
#  registration is idempotent. Does NOT change orderpricing.js / live billing.
# ============================================================================
...
# 1) back up server.js (and existing route if present)
Copy-Item (Join-Path $srv 'server.js') (Join-Path $bkDir "server.js.$ts.bak")
$routeFile = Join-Path $routes 'pricing2.js'
if (Test-Path $routeFile) { Copy-Item $routeFile (Join-Path $bkDir "pricing2.js.$ts.bak") }

# 2) write the route file (pure ASCII)
$pricing2 = @'
// routes/pricing2.js
...
'@
Set-Content -Path $routeFile -Value $pricing2 -Encoding ascii

# 3) node --check the route BEFORE touching the server
& node --check $routeFile
if ($LASTEXITCODE -ne 0) { Write-Host 'ABORT: pricing2.js failed syntax check; server NOT restarted.'; exit 1 }
Write-Host 'pricing2.js syntax OK'

# 4) register in server.js (idempotent) using UTF-8 no-BOM write
$serverPath = Join-Path $srv 'server.js'
$content = [System.IO.File]::ReadAllText($serverPath)
if ($content -notmatch 'routes/pricing2') {
  $anchor = "app.use('/api/orderpricing', require('./routes/orderpricing'));"
  if ($content.Contains($anchor)) {
    $insert  = $anchor + "`r`n" + "app.use('/api/pricing2', require('./routes/pricing2'));"
    ...
    Write-Host 'Registered /api/pricing2 in server.js'
  } else {
    Write-Host 'WARN: orderpricing anchor not found; please add this line to server.js manually:'
    Write-Host "  app.use('/api/pricing2', require('./routes/pricing2'));"
  }
} else {
  Write-Host '/api/pricing2 already registered (idempotent skip)'
}

# 5) node --check server.js BEFORE restart; restore backup on failure
& node --check $serverPath
if ($LASTEXITCODE -ne 0) {
  Write-Host 'ABORT: server.js failed syntax check after edit. Restoring backup.'
...
Write-Host ''
Write-Host 'Block E-2 deployed. Preview endpoint: GET /api/pricing2/<orderId>/preview'
Write-Host 'REVERT: delete the /api/pricing2 line from server.js, Remove-Item routes\pricing2.js, Restart-Service agi-glass'
