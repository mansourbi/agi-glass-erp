...
#  Snapshot first, then run the ADDITIVE migration (creates 8 new tables and
#  seeds 5 example profiles). Touches NO existing data; live pricing unchanged.
# ============================================================================
$ts    = Get-Date -Format 'yyyyMMdd-HHmmss'
$ckDir = 'C:\agi-server\_checkpoints'
New-Item -ItemType Directory -Force -Path $ckDir | Out-Null
$snap  = Join-Path $ckDir "agi-glass_preE1_$ts.db"

$snapjs = @'
const Database=require('C:\\agi-server\\node_modules\\better-sqlite3');
const dest=process.argv[2];
const db=new Database('C:\\agi-server\\agi-glass.db',{readonly:true});
db.backup(dest).then(()=>{const v=new Database(dest,{readonly:true});console.log('pre-E1 snapshot OK -> '+dest+'  (products='+v.prepare('SELECT COUNT(*) c FROM products').get().c+', process_pricing='+v.prepare('SELECT COUNT(*) c FROM process_pricing').get().c+')');v.close();db.close();}).catch(e=>{console.error('SNAP FAIL',e.message);process.exit(1);});
'@
$st = Join-Path $env:TEMP 'agi_snapE1.js'
...
'@
$ejs = Join-Path $ckDir 'blockE1.js'
Set-Content -Path $ejs -Value $blockE1 -Encoding ascii

Write-Host "`n--- Running Block E-1 migration ---"
node $ejs
Write-Host "`nMigration script saved at: $ejs"
Write-Host "REVERT (only if needed): node -e `"const d=require('C:\\agi-server\\node_modules\\better-sqlite3')('C:\\agi-server\\agi-glass.db');d.exec('DROP TABLE IF EXISTS order_extra_charges;DROP TABLE IF EXISTS order_price_choice;DROP TABLE IF EXISTS customer_price_profile;DROP TABLE IF EXISTS product_default_price;DROP TABLE IF EXISTS pricing_rules;DROP TABLE IF EXISTS extra_charge_categories;DROP TABLE IF EXISTS price_profiles;');console.log('Block E-1 reverted');`""
