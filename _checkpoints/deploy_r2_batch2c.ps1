# deploy_r2_batch2c.ps1  -- R-2 batch 2c: Reports + Pricing + Purchasing (portal-only)
...
$ok = ($txt -match "purchasing\.receive") -and ($txt -match "pricing\.profiles\.view") -and ($txt -match "orders\.pricing\.finalize")
Write-Host ("batch-2c markers present: " + $ok)
...
Write-Host "Batch 2c deployed (Reports + Pricing + Purchasing (portal-only))."
