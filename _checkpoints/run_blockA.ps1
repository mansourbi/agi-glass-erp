...
#  Snapshot first, then run the ADDITIVE migration (creates 3 new tables,
#  touches NO existing data). Revert command printed at the end.
# ============================================================================
$ts    = Get-Date -Format 'yyyyMMdd-HHmmss'
$ckDir = 'C:\agi-server\_checkpoints'
New-Item -ItemType Directory -Force -Path $ckDir | Out-Null
$snap  = Join-Path $ckDir "agi-glass_preA_$ts.db"
...
db.backup(dest).then(()=>{const v=new Database(dest,{readonly:true});console.log('pre-A snapshot OK -> '+dest+'  (orders='+v.prepare('SELECT COUNT(*) c FROM orders').get().c+', final_products='+v.prepare('SELECT COUNT(*) c FROM final_products').get().c+')');v.close();db.close();}).catch(e=>{console.error('SNAP FAIL',e.message);process.exit(1);});
'@
$st = Join-Path $env:TEMP 'agi_snapA.js'
Set-Content -Path $st -Value $snapjs -Encoding ascii
node $st $snap
Remove-Item $st

# 2) write the Block A migration script (pure ASCII; Arabic stored as \u escapes)
$blockA = @'
// ===========================================================================
// BLOCK A  \u2014  Catalog normalization (ID-based attributes + alias bridge)
...
'@
$ajs = Join-Path $ckDir 'blockA.js'
Set-Content -Path $ajs -Value $blockA -Encoding ascii

# 3) run it
Write-Host "`n--- Running Block A migration ---"
node $ajs
Write-Host "`nMigration script saved at: $ajs"
Write-Host "REVERT (only if needed): node -e `"const d=require('C:\\agi-server\\node_modules\\better-sqlite3')('C:\\agi-server\\agi-glass.db');d.exec('DROP TABLE IF EXISTS attribute_value_aliases;DROP TABLE IF EXISTS attribute_values;DROP TABLE IF EXISTS attributes;');console.log('Block A reverted');`""
