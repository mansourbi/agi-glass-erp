...
const r = db.prepare("SELECT id FROM roles WHERE name=?").get("ZZ_R3C_TEST");
const w = db.prepare("DELETE FROM workers WHERE email=?").run("zz_r3c");
if (r) {
  db.prepare("DELETE FROM role_permissions WHERE role_id=?").run(r.id);
  db.prepare("DELETE FROM roles WHERE id=?").run(r.id);
}
console.log("CLEANED: removed users("+w.changes+"), role ZZ_R3C_TEST"+(r?" (id "+r.id+")":" (absent)"));
'@
$tmp = "C:\agi-server\_zz_r3c_teardown.js"
